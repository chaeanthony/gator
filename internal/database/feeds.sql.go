...
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at, (SELECT name FROM users WHERE id=feeds.user_id) FROM feeds
`
...
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds WHERE user_id=(SELECT id FROM users WHERE users.name=$1)
`

func (q *Queries) GetFeedsByUserName(ctx context.Context, name string) ([]Feed, error) {
